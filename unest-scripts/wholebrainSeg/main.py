...
def main(cfig, device):
    ...
    def train(global_step,train_loader,dice_val_best, val_shape_dict, train_Dice_per_class, validation_Dice_per_class):
        model.train()
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if global_step % 40 == 0:
                ...
                for i in range(1, cfig['num_classes']):
                    ...
                    train_Dice_per_class[f"label{i}"].append(organ_Dice)
            ...
            if global_step % cfig['eval_num'] == 0:
                ...
                mean_list, validation_Dice_per_class = validation(epoch_iterator_val, val_shape_dict, validation_Dice_per_class)
        ...
        return global_step, dice_val_best, train_Dice_per_class, validation_Dice_per_class


    def validation(epoch_iterator_val, val_shape_dict, validation_Dice_per_class):
        model.eval()
        metric_values = []
        ...
        with torch.no_grad():
            for step, batch in enumerate(epoch_iterator_val):
                ...
                for i in range(1, cfig['num_classes']):
                    ...
                    dice_list_sub.append(organ_Dice)
                    validation_Dice_per_class[f"label{i}"].append(organ_Dice)
        ...
        return mean_list, validation_Dice_per_class
    ...
    train_Dice_per_class = {f"label{i}": [] for i in range(1, 28)}
    train_Mean_Dice_per_class = {f"label{i}": "" for i in range(1, 28)}
    validation_Dice_per_class = {f"label{i}": [] for i in range(1, 28)}
    validation_Mean_Dice_per_class = {f"label{i}": "" for i in range(1, 28)}

    while global_step < cfig['num_steps']:
        global_step, dice_val_best, train_Dice_per_class, validation_Dice_per_class = train(global_step,train_loader,dice_val_best, val_shape_dict, train_Dice_per_class, validation_Dice_per_class)
    checkpoint = {'global_step': global_step,'state_dict': model.state_dict(),'optimizer': optimizer.state_dict()}

    sum=0
    for key, values in train_Dice_per_class.items():
        mean_value = np.mean(values)
        train_Mean_Dice_per_class[key] = mean_value
        sum+=mean_value
    print(f"Mean of Dice score per class:{train_Mean_Dice_per_class}")
    print(f"Mean of Dice score for training set:{sum/28}")

    sum=0
    for key, values in validation_Dice_per_class.items():
        mean_value = np.mean(values)
        validation_Mean_Dice_per_class[key] = mean_value
        sum+=mean_value
    print(f"Mean of Dice score per class:{validation_Mean_Dice_per_class}")
    print(f"Mean of Dice score for validation set:{sum/28}")
    save_ckp(checkpoint, logdir+'/model_final_epoch.pt')

if __name__ == '__main__':
    yaml_file = '/home/at84490/Documents/projects/UNesT/UNesT-main14.4/wholebrainSeg/yaml/unest_large.yaml'
